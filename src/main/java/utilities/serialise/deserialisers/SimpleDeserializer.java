...
public class SimpleDeserializer {
    ...
    private static final JsonDeserializer<LayeredNeuralNetwork<SMatrix>> deserializer = (src, type, context) -> {
        ...
        SimpleInitializer init = (SimpleInitializer) ParameterInitializer.get(wM, bM, initObj.get("name").getAsString(),
                SMatrix.class);
        ...
        for (var l : layers) {
            ...
            ActivationFunction<SMatrix> act = NetworkDataCache.simpleFunctions.get(lSrc.get("activation").getAsString());
            ...
            double[][] w = new double[nestedWeight.size()][weightCols], b = new double[nestedBias.size()][1];
            ...
            NetworkLayer<SMatrix> lr = new NetworkLayer<SMatrix>(neurons, l2, act, weights, bias);
        ...
        Optimizer<SMatrix> optimizer = NetworkDataCache.simpleOptimisers.get(optimizerObj.get("name").getAsString());
    ...
    public LayeredNeuralNetwork<SMatrix> deserialise(File jsonFile) {
    ...
    public LayeredNeuralNetwork<SMatrix> deserialise(String json) {
        json = json.trim();
