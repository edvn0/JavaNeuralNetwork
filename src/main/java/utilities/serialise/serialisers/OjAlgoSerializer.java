	...
	public OjAlgoSerializer() {
		...
		gsonb.registerTypeAdapter(activationFunctions,
			new JsonSerializer<List<ActivationFunction<Primitive64Matrix>>>() {
				@Override
				public JsonElement serialize(List<ActivationFunction<Primitive64Matrix>> src,
					Type typeOfSrc,
		...
		gsonb.registerTypeAdapter(evalFunction,
			new JsonSerializer<EvaluationFunction<Primitive64Matrix>>() {
				@Override
				public JsonElement serialize(EvaluationFunction<Primitive64Matrix> src,
					Type typeOfSrc,
					JsonSerializationContext context) {
					JsonObject inner = new JsonObject();
					var info = src.params();
		...
		gsonb.registerTypeAdapter(costFunction,
			new JsonSerializer<CostFunction<Primitive64Matrix>>() {
				@Override
