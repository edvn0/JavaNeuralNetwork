...
public class SandboxMnistLayered extends AbstractDemo<Primitive64Matrix> {
	...
	protected DeepLearnable<Primitive64Matrix> createNetwork() {
		...
		var b = new LayeredNetworkBuilder<Primitive64Matrix>(28 * 28).layer(new NetworkLayer<>(f, 784, 0.01))
				.layer(new NetworkLayer<>(f, 30, 0.1)).layer(new NetworkLayer<>(f, 70, 0.1))
				.layer(new NetworkLayer<>(f, 90, 0.1)).layer(new NetworkLayer<>(f, 50, 0.1))
				.layer(new NetworkLayer<>(softMax, 10)).clipping(true).costFunction(new SmoothL1CostFunction<>())
				.evaluationFunction(new ArgMaxEvaluationFunction<>()).optimizer(new ADAM<>(0.001, 0.9, 0.999))
				.initializer(new OjAlgoInitializer(MethodConstants.XAVIER, MethodConstants.SCALAR));
		return b.create();
	}
	...
	protected void serialise(DeepLearnable<Primitive64Matrix> in) {
		...
		layeredSerializer.serialise(new File(this.outputDirectory() + "/OjAlgo_Layered_XOR_Network.json"), actual);
	}
