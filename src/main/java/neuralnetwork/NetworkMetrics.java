...
public class NetworkMetrics {
	...
	private double maxTime() {
		return Collections.max(calculationTimes);
	}

	private double minTime() {
		return Collections.min(calculationTimes);
	}
	...
	private void chartForTimeMetrics(final String path) throws IOException {
		CategoryChart cg = new CategoryChartBuilder().width(800).height(500).title("Time Metrics")
			.xAxisTitle("Time Categories")
			.yAxisTitle("Time Taken").build();
		...
		BitmapEncoder.saveBitmapWithDPI(cg,
			createChartPathFromBasePath(path, "TimeMetricsPlot") + "_" + getNow(), BitmapFormat.PNG,
			300);
