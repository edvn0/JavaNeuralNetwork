...
public class Momentum<M> implements Optimizer<M> {
	...
	public List<Matrix<M>> changeWeights(final List<Matrix<M>> weights,
		final List<Matrix<M>> deltaWeights) {
	...
	public List<Matrix<M>> changeBiases(final List<Matrix<M>> biases,
		final List<Matrix<M>> deltaBiases) {
	...
	private Matrix<M> getMomentumDeltaSingle(int i, final Matrix<M> in, final Matrix<M> deltaIns,
		...
		if (lastDeltas.get(i) == null) {
			...
			lastDeltas.set(i, lastDeltas.get(i)).multiply(momentumRate)
				.add(deltaIns.multiply(this.lR));
		}
	...
	private List<Matrix<M>> getMomentumDeltas(final List<Matrix<M>> in,
		final List<Matrix<M>> deltaIns,
		final List<Matrix<M>> lastDeltas) {
		...
		for (int i = 0; i < in.size(); i++) {
			if (lastDeltas.get(i) == null) {
				...
				lastDeltas.set(i, lastDeltas.get(i)).multiply(momentumRate)
					.add(deltaIns.get(i).multiply(this.lR));
			}
